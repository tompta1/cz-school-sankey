...
import {
  ...
  buildMvRootGraph,
  getMvBudgetAggregates,
  getMvFireRescueActivityAggregates,
...
import {
  appendEnvironmentBranch,
  getAtlasEnvironmentGraph,
  getEnvironmentBudgetEntities,
  getEnvironmentRecipientMetrics,
  getEnvironmentTotal,
} from './atlas/environment.js';
import {
  appendMmrBranch,
  getAtlasMmrGraph,
  getMmrBudgetAggregates,
  getMmrRecipientMetrics,
  getMmrTotal,
} from './atlas/mmr.js';
import {
  appendMpoBranch,
  getAtlasMpoGraph,
  getMpoBudgetEntities,
  getMpoRecipientMetric,
  getMpoTotal,
} from './atlas/mpo.js';
import {
  appendMkBranch,
  getAtlasMkGraph,
  getMkBudgetAggregates,
  getMkBudgetEntities,
  getMkProgramMetrics,
  getMkTotal,
} from './atlas/mk.js';
import {
  appendMzvBranch,
  getAtlasMzvGraph,
  getMzvAidBranchMetrics,
  getMzvBudgetEntities,
  getMzvDiplomaticMetrics,
  getMzvTotal,
} from './atlas/mzv.js';
import {
  appendMoBranch,
  getAtlasMoGraph,
  getMoBudgetAggregates,
  getMoBudgetEntities,
...
function sumReportedProcedures(rows: HealthFinanceRow[]): number {
...
function createZzsNode(rows: HealthFinanceRow[], activity: HealthZzsActivityAggregate | null): AtlasNode {
  return {
    ...
    level: 2,
    metadata: {
      capacity: activity?.departuresTotal ?? null,
      providerCount: rows.length,
      focus: 'zzs',
...
function createRegionNode(
  ...
  const capacity = rows.some((row) => row.focus === 'hospital')
    ? sumReportedProcedures(rows) || null
    : null;
  return {
    id,
    name,
...
function createProviderNode(row: HealthFinanceRow, level: number): AtlasNode {
  return {
    ...
    metadata: {
      capacity: row.focus === 'hospital' ? row.totalQuantity || null : null,
...
async function getHealthZzsActivityAggregate(year: number): Promise<HealthZzsActivityAggregate | null> {
  ...
  if (aggregate.departuresTotal <= 0) return null;
  return aggregate;
}
...
function buildCombinedRootGraph(
  ...
  mmrBudgetRows: Awaited<ReturnType<typeof getMmrBudgetAggregates>>,
  mmrRecipientMetrics: Awaited<ReturnType<typeof getMmrRecipientMetrics>>,
  mpoBudgetRows: Awaited<ReturnType<typeof getMpoBudgetEntities>>,
  mpoRecipientMetric: Awaited<ReturnType<typeof getMpoRecipientMetric>>,
  mkBudgetRows: Awaited<ReturnType<typeof getMkBudgetEntities>>,
  mkBudgetAggregates: Awaited<ReturnType<typeof getMkBudgetAggregates>>,
  mkProgramMetrics: Awaited<ReturnType<typeof getMkProgramMetrics>>,
  mzvBudgetRows: Awaited<ReturnType<typeof getMzvBudgetEntities>>,
  mzvDiplomaticMetrics: Awaited<ReturnType<typeof getMzvDiplomaticMetrics>>,
  mzvAidBranchMetrics: Awaited<ReturnType<typeof getMzvAidBranchMetrics>>,
  ...
  environmentRecipientMetrics: Awaited<ReturnType<typeof getEnvironmentRecipientMetrics>>,
  ...
  const mmrTotal = getMmrTotal(mmrBudgetRows);
  const mpoTotal = getMpoTotal(mpoBudgetRows);
  ...
  appendMmrBranch(nodes, links, year, mmrBudgetRows, mmrRecipientMetrics);
  appendMpoBranch(nodes, links, year, mpoBudgetRows, mpoRecipientMetric);
  ...
  if (hospitalRows.length > 0) {
    ...
    addNode(nodes, { ...createInsuranceNode(sumReportedProcedures(hospitalRows) || null), level: 0 });
    ...
    for (const group of ownerGroups) {
      ...
      addNode(nodes, {
        ...
        level: 2,
        metadata: {
          capacity: sumReportedProcedures(group.rows) || null,
  ...
  if (zzsAmount > 0) {
    addNode(nodes, createZzsMixedSourceNode(zzsActivity?.departuresTotal ?? null));
...
function buildHealthRootGraph(
  ...
  if (hospitalRows.length > 0) {
    addNode(nodes, createInsuranceNode(sumReportedProcedures(hospitalRows) || null));
...
function buildOwnerRegionGraph(year: number, ownerBranch: OwnerBranch, rows: HealthFinanceRow[]) {
  ...
  addNode(nodes, createInsuranceNode(sumReportedProcedures(ownerRows) || null));
  addNode(nodes, {
    id: owner.id,
    name: owner.name,
    ...
    level: 2,
    metadata: {
      capacity: sumReportedProcedures(ownerRows) || null,
...
function buildZzsRegionGraph(year: number, rows: HealthFinanceRow[], activity: HealthZzsActivityAggregate | null) {
  ...
  addNode(nodes, createZzsMixedSourceNode(activity?.departuresTotal ?? null));
...
function buildRegionProviderGraph(year: number, branchKey: string, regionName: string, rows: HealthFinanceRow[]) {
  ...
  const activityCapacity = branchKey === 'public_health' || branchKey === 'zzs'
    ? null
    : sumReportedProcedures(regionRows) || null;
  const regionId = regionNodeId(branchKey, regionName);

  addNode(nodes, createCostsNode(activityCapacity));
  ...
  if (branchKey === 'public_health') {
    ...
    addNode(nodes, createZzsMixedSourceNode());
    ...
    addNode(nodes, createInsuranceNode(activityCapacity));
    addNode(nodes, {
      id: owner.id,
      name: owner.name,
      category: owner.category,
  ...
  for (const row of regionRows.sort((a, b) => b.costs - a.costs)) {
    ...
    links.push(
      makeLink(
        ...
        row.costs,
        year,
        row.focus === 'hospital' ? 'health_hospital_provider_costs' : 'health_provider_costs',
    ...
    links.push(
      makeLink(
        ...
        row.costs,
        year,
        row.focus === 'hospital' ? 'health_hospital_operating_costs' : 'health_operating_costs',
        providerCostNote(row),
        row.sourceDataset,
      ),
    );
...
function buildProviderDetailGraph(year: number, providerIco: string, rows: HealthFinanceRow[]) {
  ...
  const regionId = regionNodeId(branchKey, regionName);
  const capacity = row.focus === 'hospital' ? row.totalQuantity || null : null;
...
export { getAtlasEnvironmentGraph };
export { getAtlasMmrGraph };
export { getAtlasMpoGraph };
