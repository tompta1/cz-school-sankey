...
const MF_ROOT_ID = 'mf:ministry:mf';
...
function isClickableMpoNode(node: SankeyNode): boolean {
  if (node.id === MPO_ROOT_ID) return true;
  if (node.id === MPO_SUPPORT_ID) return true;
  return false;
}
...
function isClickableMfNode(node: SankeyNode): boolean {
  return node.id === MF_ROOT_ID;
}
...
export function AtlasDashboard() {
  ...
  useEffect(() => {
    ...
    } else if (currentView.scope === 'mf') {
      const params = new URLSearchParams({ year: String(selectedYear) });
      if (currentView.nodeId) params.set('nodeId', currentView.nodeId);
      path = `/api/atlas/mf?${params.toString()}`;
...
function handleMvNodeClick(node: SankeyNode) {
  ...
  function handleMpoNodeClick(node: SankeyNode) {
    if (node.id === PREV_WINDOW_ID || node.id === NEXT_WINDOW_ID) {
      setViewStack((prev) => {
        const current = prev.at(-1);
        if (!current || current.scope !== 'mpo') return prev;
        const step = node.id === PREV_WINDOW_ID ? -28 : 28;
        return pageAtlasView(prev, step);
      });
      return;
    ...
    if (!isClickableMpoNode(node)) return;
    const nextNodeId = node.id === MPO_ROOT_ID ? null : node.id;
  ...
  function handleMfNodeClick(node: SankeyNode) {
    if (!isClickableMfNode(node)) return;
    const nextNodeId = node.id === MF_ROOT_ID ? null : node.id;
    setViewStack((prev) => pushAtlasView(prev, { scope: 'mf', nodeId: nextNodeId, label: node.name, offset: 0 }));
  }
  ...
  function handleNodeClick(nodeId: string) {
    ...
    if (currentView.scope === 'root') {
      ...
      if (node.id.startsWith('mf:')) {
        handleMfNodeClick(node);
        return;
      }
