...
export function SchoolDashboard() {
  ...
  function handleNodeClick(nodeId: string) {
    if (nodeId === PREV_WINDOW_ID || nodeId === NEXT_WINDOW_ID) {
      setDrilldownStack((prev) => {
        ...
        const step = nodeId === PREV_WINDOW_ID ? -pageSize : pageSize;
        const newOffset = Math.max(0, (last.offset ?? 0) + step);
  ...
  const backLabel = drilldownStack.length > 1 ? drilldownStack.at(-2)!.label : ROOT_TITLE;
        ...
        <div className="topbar__main">
          ...
          <div className="topbar__hover">
            {hoverInfo ? (
              ...
              <span className="topbar__hover-context">
                {drilldownStack.map((entry) => entry.label).join(' › ')}
              </span>
            ) : null}
          </div>
          ...
          <div className="topbar__controls">
            <button
              ...
              onClick={() => setPerPupil((value) => !value)}
              title="Přepnout pohled Kč/žák (kapacita RSSZ)"
            >
              {perPupil ? 'Kč/žák/rok' : 'celkem'}
            </button>
            {prevGraph && prevYear && (
              <button className="year-toggle-btn" onClick={() => setPrevActive((value) => !value)}>
