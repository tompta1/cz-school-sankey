...
DATASET_CODE = "mzv_budget_entities"
MONITOR_BASE_URL = "https://monitor.statnipokladna.gov.cz"
MONITOR_API_URL = f"{MONITOR_BASE_URL}/api/ukazatele"
OUTPUT_FILE_NAME = "monitor-ukazatele-mzv-entities.csv"

MZV_ENTITIES = [
    {
        "entity_ico": "45769851",
        "entity_name": "Ministerstvo zahraničních věcí",
        "entity_kind": "foreign_affairs_ministry",
    },
]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch MZV budget entities from Monitor MF")
    ...
    *,
    ...
    write_sidecar(
        data_path,
        {
            ...
            "years": years,
            "entity_count": len(MZV_ENTITIES),
            "request_count": len(MZV_ENTITIES) * len(years),
            "row_count": len(rows),
            "failure_count": len(failures),
            "failures": failures[:100],
            "generator": "etl/mzv/fetch_budget_entities.py",
...
def main() -> None:
    ...
    rows: list[dict[str, object]] = []
    failures: list[str] = []
    jobs = [(entity, year) for year in years for entity in MZV_ENTITIES]
    ...
    print(f"Entities queried: {len(MZV_ENTITIES)}")
