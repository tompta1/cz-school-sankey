...
DATASETS = [
    ...
    {
        "table": "raw.school_cost_profile",
        "dataset_code": "school_cost_profiles",
        "source_code": "monitor",
        "filename": "school_costs.csv",
        "columns": [
            "dataset_release_id",
            "reporting_year",
            "institution_id",
        ...
        "build_rows": lambda release_id, year, rows, path: [
            (
                ...
                row["institution_id"],
                row.get("ico") or None,
                int(row.get("total_costs_amount") or 0),
                int(row.get("materials_amount") or 0),
                int(row.get("energy_amount") or 0),
                int(row.get("repairs_amount") or 0),
                int(row.get("services_amount") or 0),
                int(row.get("personnel_amount") or 0),
                int(row.get("depreciation_amount") or 0),
                int(row.get("other_costs_amount") or 0),
                row.get("basis") or None,
                row.get("certainty") or None,
                row.get("note") or None,
                Jsonb({"local_path": str(path.relative_to(ROOT))}),
