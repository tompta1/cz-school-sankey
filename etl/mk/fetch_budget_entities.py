...
DATASET_CODE = "mk_budget_entities"
MONITOR_BASE_URL = "https://monitor.statnipokladna.gov.cz"
MONITOR_API_URL = f"{MONITOR_BASE_URL}/api/ukazatele"
OUTPUT_FILE_NAME = "monitor-ukazatele-mk.csv"

MK_ENTITIES = [
    {
        "entity_ico": "00023671",
        "entity_name": "Ministerstvo kultury",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch MK budget entity from Monitor MF")
    ...
    *,
    ...
    write_sidecar(
        data_path,
        {
            ...
            "years": years,
            "entity_count": len(MK_ENTITIES),
            "request_count": len(MK_ENTITIES) * len(years),
            "row_count": len(rows),
            "failure_count": len(failures),
            "failures": failures[:100],
            "generator": "etl/mk/fetch_budget_entities.py",
...
def main() -> None:
    ...
    rows: list[dict[str, object]] = []
    failures: list[str] = []
    jobs = [(entity, year) for year in years for entity in MK_ENTITIES]
    ...
    print(f"Entities queried: {len(MK_ENTITIES)}")
