...
DATASET_CODE = "mf_budget_entities"
MONITOR_BASE_URL = "https://monitor.statnipokladna.gov.cz"
MONITOR_API_URL = f"{MONITOR_BASE_URL}/api/ukazatele"
OUTPUT_FILE_NAME = "monitor-ukazatele-mf-entities.csv"

MF_ENTITIES = [
    {
        "entity_ico": "00006947",
        "entity_name": "Ministerstvo financí",
        "entity_kind": "ministry_core",
    },
    {
        "entity_ico": "72080043",
        "entity_name": "Generální finanční ředitelství",
        "entity_kind": "tax_admin",
    },
    {
        "entity_ico": "71214011",
        "entity_name": "Generální ředitelství cel",
        "entity_kind": "customs",
    },
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch MF budget entities from Monitor MF")
    ...
    *,
    ...
    write_sidecar(
        data_path,
        {
            ...
            "years": years,
            "entity_count": len(MF_ENTITIES),
            "request_count": len(MF_ENTITIES) * len(years),
            "row_count": len(rows),
            "failure_count": len(failures),
            "failures": failures[:100],
            "generator": "etl/mf/fetch_budget_entities.py",
...
def main() -> None:
    ...
    rows: list[dict[str, object]] = []
    failures: list[str] = []
    jobs = [(entity, year) for year in years for entity in MF_ENTITIES]
    ...
    print(f"Entities queried: {len(MF_ENTITIES)}")
