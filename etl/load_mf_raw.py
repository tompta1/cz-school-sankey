...
RAW_ROOT = ROOT / "etl" / "data" / "raw" / "mf"

SOURCES = {
    "mf_budget_entities": ("Monitor MF", "https://monitor.statnipokladna.gov.cz"),
    "mf_activity_metrics": ("Finanční správa ČR výroční zprávy", "https://www.financnisprava.cz"),
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Load downloaded MF files into raw Postgres tables")
    parser.add_argument(
        "--database-url",
    ...
    *,
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            values (%s, 'mf', %s, %s, %s, %s, %s, %s, 0, %s, 'staged')
...
def load_budget_entities(conn: psycopg.Connection, dataset_release_id: int, path: Path) -> int:
    ...
    with conn.cursor() as cur:
        cur.execute("delete from raw.mf_budget_entity where dataset_release_id = %s", (dataset_release_id,))
        cur.executemany(
            """
            insert into raw.mf_budget_entity (
              dataset_release_id,
              reporting_year,
              period_code,
              entity_ico,
...
def load_activity_metrics(conn: psycopg.Connection, dataset_release_id: int, path: Path) -> int:
    rows = []
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        for row in csv.DictReader(handle):
            rows.append(
    ...
    with conn.cursor() as cur:
        cur.execute("delete from raw.mf_activity_metric where dataset_release_id = %s", (dataset_release_id,))
        cur.executemany(
            """
            insert into raw.mf_activity_metric (
              dataset_release_id,
              reporting_year,
              metric_code,
              metric_name,
...
LOADERS = {
    "mf_budget_entities": load_budget_entities,
    "mf_activity_metrics": load_activity_metrics,
}
