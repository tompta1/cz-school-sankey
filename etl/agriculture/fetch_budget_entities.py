...
OUTPUT_FILE_NAME = "monitor-ukazatele-agriculture-entities.csv"

AGRICULTURE_ENTITIES = [
    {
        "entity_ico": "00020478",
        "entity_name": "Ministerstvo zemědělství",
        "entity_kind": "ministry_admin",
    },
    {
        "entity_ico": "48133981",
        "entity_name": "Státní zemědělský intervenční fond",
        "entity_kind": "intervention_fund_admin",
    },
]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch agriculture-sector budget entities from Monitor MF")
    ...
    *,
    ...
    write_sidecar(
        data_path,
        {
            ...
            "years": years,
            "entity_count": len(AGRICULTURE_ENTITIES),
            "request_count": len(AGRICULTURE_ENTITIES) * len(years),
            "row_count": len(rows),
            "failure_count": len(failures),
            "failures": failures[:100],
            "generator": "etl/agriculture/fetch_budget_entities.py",
...
def main() -> None:
    ...
    rows: list[dict[str, object]] = []
    failures: list[str] = []
    jobs = [(entity, year) for year in years for entity in AGRICULTURE_ENTITIES]
    ...
    print(f"Entities queried: {len(AGRICULTURE_ENTITIES)}")
