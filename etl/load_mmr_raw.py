...
RAW_ROOT = ROOT / "etl" / "data" / "raw" / "mmr"

SOURCES = {
    "mmr_budget_aggregates": ("Ministerstvo pro místní rozvoj", "https://mmr.gov.cz"),
    "mmr_irop_operations": ("DotaceEU / IROP", "https://www.dotaceeu.cz"),
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Load downloaded MMR files into raw Postgres tables")
    parser.add_argument(
        "--database-url",
    ...
    parser.add_argument(
        "--dataset",
        action="append",
        choices=["mmr_budget_aggregates", "mmr_irop_operations"],
    ...
    *,
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            values (%s, 'mmr', %s, %s, %s, %s, %s, %s, 0, %s, 'staged')
...
def load_budget_aggregates(conn: psycopg.Connection, dataset_release_id: int, path: Path) -> int:
    ...
    with conn.cursor() as cur:
        cur.execute("delete from raw.mmr_budget_aggregate where dataset_release_id = %s", (dataset_release_id,))
        cur.executemany(
            """
            insert into raw.mmr_budget_aggregate (
              dataset_release_id,
              reporting_year,
              metric_code,
              metric_name,
...
def load_irop_operations(conn: psycopg.Connection, dataset_release_id: int, path: Path) -> int:
    with conn.cursor() as cur:
        cur.execute("delete from raw.mmr_irop_operation_yearly where dataset_release_id = %s", (dataset_release_id,))
        cur.execute(
            """
            create temporary table mmr_irop_operation_stage (
        ...
        cur.execute(
            """
            insert into raw.mmr_irop_operation_yearly (
              dataset_release_id,
              reporting_year,
              branch_code,
              branch_name,
...
def load_dataset(conn: psycopg.Connection, dataset_code: str, data_path: Path) -> int:
    ...
    if dataset_code == "mmr_budget_aggregates":
        row_count = load_budget_aggregates(conn, dataset_release_id, data_path)
    elif dataset_code == "mmr_irop_operations":
        row_count = load_irop_operations(conn, dataset_release_id, data_path)
    else:
