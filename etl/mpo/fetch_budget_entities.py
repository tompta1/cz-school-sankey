...
DATASET_CODE = "mpo_budget_entities"
MONITOR_BASE_URL = "https://monitor.statnipokladna.gov.cz"
MONITOR_API_URL = f"{MONITOR_BASE_URL}/api/ukazatele"
OUTPUT_FILE_NAME = "monitor-ukazatele-mpo.csv"

MPO_ENTITIES = [
    {
        "entity_ico": "47609109",
        "entity_name": "Ministerstvo průmyslu a obchodu",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch MPO budget entity from Monitor MF")
    ...
    *,
    ...
    write_sidecar(
        data_path,
        {
            ...
            "years": years,
            "entity_count": len(MPO_ENTITIES),
            "request_count": len(MPO_ENTITIES) * len(years),
            "row_count": len(rows),
            "failure_count": len(failures),
            "failures": failures[:100],
            "generator": "etl/mpo/fetch_budget_entities.py",
...
def main() -> None:
    ...
    rows: list[dict[str, object]] = []
    failures: list[str] = []
    jobs = [(entity, year) for year in years for entity in MPO_ENTITIES]
    ...
    print(f"Entities queried: {len(MPO_ENTITIES)}")
