...
DATASET_CODE = "mo_personnel_metrics"
OUTPUT_FILE_NAME = "mo-personnel-metrics.csv"
FACTS_TRENDS_PDF_URL = "https://mocr.mo.gov.cz/assets/finance-a-zakazky/resortni-rozpocet/fakta-a-trendy-2025.pdf"
METRIC_CODE = "PROFESSIONAL_SOLDIERS"
METRIC_NAME = "Počty vojáků z povolání"
NUMBER_PATTERN = r"(\d{1,3}(?: \d{3})*)"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch MO personnel metrics from Fakta a trendy 2025")
...
def extract_counts(text: str) -> dict[int, int]:
    pattern = re.compile(
        rf"{re.escape(METRIC_NAME)}\s+{NUMBER_PATTERN}\s+{NUMBER_PATTERN}\s+{NUMBER_PATTERN}\s+{NUMBER_PATTERN}\s+{NUMBER_PATTERN}",
        flags=re.IGNORECASE,
    )
    match = pattern.search(text)
    if not match:
        raise RuntimeError(f"Could not parse personnel row for {METRIC_NAME}")
...
def main() -> None:
    ...
    if unsupported:
        raise SystemExit(f"MO personnel metrics currently support only 2024 and 2025, got: {', '.join(map(str, unsupported))}")
    ...
    write_sidecar(
        data_path,
        {
            ...
            "generator": "etl/mo/fetch_personnel_metrics.py",
        },
    )
