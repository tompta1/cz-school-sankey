...
DATASET_CODE = "mo_budget_entities"
MONITOR_BASE_URL = "https://monitor.statnipokladna.gov.cz"
MONITOR_API_URL = f"{MONITOR_BASE_URL}/api/ukazatele"
OUTPUT_FILE_NAME = "monitor-ukazatele-mo-entities.csv"

MO_ENTITIES = [
    {
        "entity_ico": "60162694",
        "entity_name": "Ministerstvo obrany",
        "entity_kind": "defense_ministry",
    },
]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch MO budget entities from Monitor MF")
    ...
    *,
    ...
    write_sidecar(
        data_path,
        {
            ...
            "years": years,
            "entity_count": len(MO_ENTITIES),
            "request_count": len(MO_ENTITIES) * len(years),
            "row_count": len(rows),
            "failure_count": len(failures),
            "failures": failures[:100],
            "generator": "etl/mo/fetch_budget_entities.py",
...
def main() -> None:
    ...
    rows: list[dict[str, object]] = []
    failures: list[str] = []
    jobs = [(entity, year) for year in years for entity in MO_ENTITIES]
    ...
    print(f"Entities queried: {len(MO_ENTITIES)}")
