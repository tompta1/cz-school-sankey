...
DATASET_CODE = "environment_budget_entities"
MONITOR_BASE_URL = "https://monitor.statnipokladna.gov.cz"
MONITOR_API_URL = f"{MONITOR_BASE_URL}/api/ukazatele"
OUTPUT_FILE_NAME = "monitor-ukazatele-environment-entities.csv"

ENVIRONMENT_ENTITIES = [
    {
        "entity_ico": "00164801",
        "entity_name": "Ministerstvo životního prostředí",
        "entity_kind": "ministry_admin",
    },
    {
        "entity_ico": "00020729",
        "entity_name": "Státní fond životního prostředí České republiky",
        "entity_kind": "environment_fund_admin",
    },
]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Fetch MŽP/SFŽP budget entities from Monitor MF")
    ...
    *,
    ...
    write_sidecar(
        data_path,
        {
            ...
            "years": years,
            "entity_count": len(ENVIRONMENT_ENTITIES),
            "request_count": len(ENVIRONMENT_ENTITIES) * len(years),
            "row_count": len(rows),
            "failure_count": len(failures),
            "failures": failures[:100],
            "generator": "etl/environment/fetch_budget_entities.py",
...
def main() -> None:
    ...
    rows: list[dict[str, object]] = []
    failures: list[str] = []
    jobs = [(entity, year) for year in years for entity in ENVIRONMENT_ENTITIES]
    ...
    print(f"Entities queried: {len(ENVIRONMENT_ENTITIES)}")
