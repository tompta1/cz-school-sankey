...
create table if not exists raw.mf_budget_entity (
  raw_id bigserial primary key,
  dataset_release_id bigint not null references meta.dataset_release(dataset_release_id) on delete cascade,
  reporting_year integer not null,
  period_code text not null,
...
create index if not exists mf_budget_entity_year_ico_idx
  on raw.mf_budget_entity (reporting_year, entity_ico);

create table if not exists raw.mf_activity_metric (
  raw_id bigserial primary key,
  dataset_release_id bigint not null references meta.dataset_release(dataset_release_id) on delete cascade,
  reporting_year integer not null,
  metric_code text not null,
...
create index if not exists mf_activity_metric_year_metric_idx
  on raw.mf_activity_metric (reporting_year, metric_code);

create or replace view mart.mf_budget_entity_latest as
select distinct on (r.reporting_year, r.entity_ico)
  r.reporting_year,
  r.period_code,
  r.entity_ico,
...
create or replace view mart.mf_activity_metric_latest as
select distinct on (r.reporting_year, r.metric_code)
  r.reporting_year,
  r.metric_code,
  r.metric_name,
